...
public class UserService {
    ...
    public User register(User user) {
        ...
        if(this.repository.findByEmail(user.getEmail())==null){
            logClient.sendLog("Info", appName,  "New user signed up");
            return this.repository.save(user);
        }
        logClient.sendLog( "Warning", appName,  "Attempt to register a new user failed.");
        return new User();
    }
    ...
    public User login(LoginRequest user) {
        ...
        if(user1==null)
        {
            logClient.sendLog("Warning", appName,  "User authentication failed  for " + user.getEmail());
            return new User();
        }
        ...
        if(((BCrypt.hashpw(user.getPassword(), salt)).equals(password)))
        {
            ...
            logClient.sendLog( "Info", appName,  "User " + user.getEmail() + " logged in.");
            return user1;
        }
        logClient.sendLog("Warning", appName,  "User authentication failed  for " + user.getEmail());
        return new User();
